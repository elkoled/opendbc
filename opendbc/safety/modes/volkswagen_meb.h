...
// Curvature steering control (HCA_03): mode-local helpers + state, kept out of shared safety/lateral.h
...
typedef struct {
  ...
  const bool inactive_curvature_is_zero;  // if false, enforces curvature near meas when disabled
  const int max_power;
...
static bool steer_curvature_cmd_checks_average(int desired_curvature, bool steer_control_enabled, const CurvatureSteeringLimits limits) {
  ...
  if (controls_allowed && steer_control_enabled) {
    ...
    // ISO lateral acceleration limit (with average road roll)
...
static safety_config volkswagen_meb_init(uint16_t param) {
  // Stock-long, lateral-only TX set: steering curvature, cancel button passthrough on bus 0 and 2,
  // lane-departure HUD passthrough, EA HUD/control passthrough, and capacitive steering wheel touch passthrough.
...
static void volkswagen_meb_rx_hook(const CANPacket_t *msg) {
  if (msg->bus == 0U) {
    ...
    // Measured curvature feedback (QFK_01.Curvature, sign in bit 55)
    ...
    // Motor_51.TSK_Status (bits 88..90) drives cruise state + accel pedal
    ...
    // Cruise buttons. With OP-long, the falling edge of Set/Resume engages while ACC main is on.
    // Cancel always disengages.
