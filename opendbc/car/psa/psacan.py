...
def create_HS2_DYN1_MDD_ETAT_2B6(packer, frame: int, accel: float, enabled: bool, gasPressed: bool, braking: int, brakePressed: int, standstill: bool, torque: int):
  ...
  values = {
    ...
    'ACC_STATUS': (5 if gasPressed else 2 if brakePressed and not standstill else 4) if enabled else (2 if brakePressed and not standstill else 3), # TODO: check transition from standing brake pressed to driving etc
