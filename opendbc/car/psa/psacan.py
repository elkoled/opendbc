...
def create_HS2_DYN1_MDD_ETAT_2B6(packer, frame: int, accel: float, enabled: bool, gasPressed: bool, braking: int, brakePressed: int, standstill: bool, torque: int):
  ...
  values = {
    ...
    'WHEEL_TORQUE_REQUEST': (0 if braking else 2) if enabled else 0, # TODO:  REVERT TO HIGH RANGE! test 1: high torque range 2: low torque range
...
# Radar, 50 Hz
def create_HS2_DYN_MDD_ETAT_2F6(packer, braking, enabled):
  values = {
    'TARGET_DETECTED': 1 if enabled else 0, # TODO: <target detected>
    ...
    'INTER_VEHICLE_DISTANCE': 50.0 if enabled else 255.5, # TODO: <distance> if enabled else 255.5,
    ...
    'DISPLAY_INTERVEHICLE_TIME': 5.0 if enabled else 6.2, # TODO: <time to vehicle> if enabled else 6.2,
    'MDD_DECEL_CONTROL_REQ': int(braking),
    'AUTO_BRAKING_STATUS': 3, # AEB # TODO: testing ALWAYS ENABLED to resolve DTC errors if enabled else 3, # maybe disabled on too high steering angle
