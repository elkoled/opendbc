...
class CarController(CarControllerBase):
  ...
  def update(self, CC, CS, now_nanos):
      ...
      if self.CP.flags & VolkswagenFlags.MEB:
        ...
        if CC.latActive:
          ...
          if self.steering_power_last > 0: # keep HCA alive until steering power has reduced to zero
            hca_enabled = True
            apply_curvature = float(np.clip(CS.measured_curvature, -self.CCP.CURVATURE_MAX, self.CCP.CURVATURE_MAX)) # synchronize with current curvature
    ...
    # Emergency Assist intervention (MEB capacitive steering wheel touch passthrough)
    ...
    # "Wechselblinken" means switching between hazards and one sided indicators for every indicator cycle (VW MEB full cycle: 0.8 seconds, 1st normal, 2nd hazards) # noqa: E501
    ...
    if self.CP.flags & VolkswagenFlags.MEB:
      if self.frame % 2 == 0:
        ...
        can_sends.append(mebcan.create_blinker_control(self.packer_pt, self.CAN.pt, CS.ea_hud_stock_values, CS.ea_control_stock_values, left_blinker, right_blinker, self.hide_ea_error)) # noqa: E501
    ...
    if self.CP.flags & VolkswagenFlags.MEB:
      ...
      if gra_send_ready and self.CP.pcmCruise:
