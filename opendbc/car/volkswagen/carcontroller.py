  ...
  def update(self, CC, CS, now_nanos):
    ...
    if self.frame % self.CCP.STEER_STEP == 0:
      if self.CP.flags & VolkswagenFlags.MEB:
        if CC.latActive:
          hca_enabled = True
          apply_curvature = actuators.curvature + (CS.steering_curvature_measured - CC.currentCurvature)
          apply_curvature = apply_std_curvature_limits(apply_curvature, self.apply_curvature_last,
                                                       CS.out.vEgoRaw, CS.steering_curvature_measured,
                                                       CS.out.steeringPressed, self.CCP.STEER_STEP,
                                                       CC.latActive, self.CCP.CURVATURE_LIMITS)
          min_power = max(self.steering_power_last - self.CCP.STEERING_POWER_STEP, self.CCP.STEERING_POWER_MIN)
          max_power = min(self.steering_power_last + self.CCP.STEERING_POWER_STEP, self.CCP.STEERING_POWER_MAX)
          steering_power = min(max(self.CCP.STEERING_POWER_MAX, min_power), max_power)
        else:
          if self.steering_power_last > 0:
            hca_enabled = True
            apply_curvature = float(np.clip(CS.steering_curvature_measured,
