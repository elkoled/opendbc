...
class CarState(CarStateBase):
  ...
  def update(self, can_parsers) -> structs.CarState:
    ...
    if self.CP.flags & VolkswagenFlags.PQ:
      ...
      return self.update_meb(pt_cp, cam_cp)
  ...
  def update_meb(self, pt_cp, cam_cp) -> structs.CarState:
    ret = structs.CarState()
    ...
    # TSK transiently faults during EPB, suppress so engage attempts immediately disable
  ...
  def update_hca_state(self, hca_status, drive_mode=True):
    # FAULT is temporary for worst likely EPS recovery time
