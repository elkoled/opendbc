...
class CarState(CarStateBase):
  def __init__(self, CP):
    ...
    self.acc_type = 0
    # MEB-specific instance state (curvature loop closure / EA spoof / blinker / TA)
  ...
  def update_meb(self, pt_cp, cam_cp, alt_cp, ext_cp) -> structs.CarState:
    ...
    # Infostufe: BSM LED on, Warnung: BSM LED flashing
    if self.CP.enableBsm:
      blindspot_driver    = bool(ext_cp.vl["MEB_Side_Assist_01"]["Blind_Spot_Info_Driver"]) or bool(ext_cp.vl["MEB_Side_Assist_01"]["Blind_Spot_Warn_Driver"])
      blindspot_passenger = bool(ext_cp.vl["MEB_Side_Assist_01"]["Blind_Spot_Info_Passenger"]) or bool(ext_cp.vl["MEB_Side_Assist_01"]["Blind_Spot_Warn_Passenger"]) # noqa: E501
      # LHD only — left = driver, right = passenger
      ret.leftBlindspot  = blindspot_driver
      ret.rightBlindspot = blindspot_passenger
    ...
    self.radar_disable_failed = True if RADAR_DISABLE_STATE["error"] and self.CP.flags & VolkswagenFlags.DISABLE_RADAR else False
