ACCEL_INACTIVE = 3.01  # one increment above the active range, marks ACC as idle to drivetrain
ACC_CTRL_ERROR    = 6
ACC_CTRL_OVERRIDE = 4
ACC_CTRL_ACTIVE   = 3
ACC_CTRL_ENABLED  = 2
...
def acc_hold_type(acc_faulted, long_active, starting, stopping, esp_hold, override):
  if acc_faulted or not long_active or override:
...
def create_acc_accel_control(packer, bus, acc_type, acc_enabled, accel, acc_control, acc_hold,
  ...
  if not acc_enabled or full_stop:
    acceleration = ACCEL_INACTIVE
  elif override:
    acceleration = 0.0  # TSK requires accel=0 during driver gas override
  else:
    acceleration = accel
