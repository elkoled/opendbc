...
def acc_hold_type(acc_faulted, long_active, starting, stopping, esp_hold, override, override_begin, long_disabling):
  # Emit RAMP_RELEASE for the first ~5 frames after override starts or long control disables,
  # otherwise the EPB faults during low-speed transitions.
  if acc_faulted:
...
def create_acc_accel_control(packer, bus, acc_type, acc_enabled, accel, acc_control, acc_hold,
  ...
  # ACC_Sollbeschleunigung_02 must be ACCEL_OVERRIDE (0.0) during gas-press (stock radar holds
  # a live accel during override) and ACCEL_INACTIVE at full stop (non-neutral accel faults).
...
def acc_control_value(main_switch_on, acc_faulted, long_active, override):
  # OVERRIDE keeps the controller alive while the driver gases so the next active frame
  # doesn't get rejected as a re-engage. Only emit ERROR while actively controlling.
  if long_active:
    if acc_faulted:
...
def create_acc_hud_control(packer, bus, acc_control, set_speed):
  # MEB_ACC_01 — replaces the stock radar's HUD frame so the cluster shows the openpilot
  # set-speed instead of a stock-ACC fault during alpha-long.
  values = {
    "ACC_Status_ACC":      acc_control,
