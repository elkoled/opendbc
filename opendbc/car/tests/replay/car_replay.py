...
def format_diff(diffs):
  ...
  for rdiffs in ranges:
    ...
    lines.append(f"\n  frames {t0}-{t1-1} (t={ts_start/1e9:.2f}s - {ts_end/1e9:.2f}s)")
    pad = 12
    ...
    if b_rises and m_rises:
      ...
      if delta:
        lines.append(" " * pad + f"rise: PR {'lags' if delta > 0 else 'leads'} by {abs(delta)} frames ({int(abs(delta) * frame_ms)}ms)")
    if b_falls and m_falls:
      delta = m_falls[0] - b_falls[0]
      if delta:
        lines.append(" " * pad + f"fall: PR {'lags' if delta > 0 else 'leads'} by {abs(delta)} frames ({int(abs(delta) * frame_ms)}ms)")
